...
> ✅ **本项目确定的平台版本（2026-09-22 与需求方确认）**：
> **旧版「MQTT物联网套件 / 多协议接入」——数据流-数据点模式**。
> 也就是说：**不需要**在控制台定义物模型/属性，直接报 `{"id":…,"dp":{数据流:[{v,t}]}}`。
>
> **30 秒判据（打开控制台一对照就知道自己在哪套平台上）**：
>
> | 你看到的 | 结论 | 本项目是否适用 |
> | --- | --- | --- |
> | 侧栏/入口有「**多协议接入**」；产品里叫「**数据流**」；建设备时**不用**先定义属性 | **旧版（本项目用的）** | ✅ 直接用 |
> | 顶部是「**OneNET Studio**」；建产品时要求**先定义物模型/属性**；数据叫「属性/事件」 | Studio（新版） | ⚠️ 需另写适配器（鉴权可复用，上报格式不同） |
> | 控制台里能看到 `$sys/{pid}/{device}/dp/post/json` 这类主题说明 | 旧版 | ✅ 直接用 |
>
> 如果你打开的控制台与本文描述**不一致**（比如让你先"定义物模型"），
> 说明你在 Studio 上 —— 那是**另一套上报格式**，请告诉负责同学（`net/onenet.py` 顶部写了差异表），
> 不要在旧版代码上硬改。
