...
> 写驱动的具体规范在 [`docs/手册/04-驱动开发规范与贡献指南.md`](docs/手册/04-驱动开发规范与贡献指南.md)。
...
1. **不要改 `rpi/health_monitor/hal/`**（契约层）。确需修改：先改
   `docs/手册/02-接口规格说明书.md` + `docs/CHANGELOG-接口.md`，通知全组，再改代码。
2. **改 HTTP 接口必须同步** `docs/手册/05-安卓通信协议.md`（只改一边算缺陷）。
3. **加/改报警码必须同时改三处**：`rpi/health_monitor/hal/models.py` 的 `AlarmCode`、
   `docs/手册/08-报警规则表.md`、安卓端 `AlarmCatalog.kt`。
...
进度表在 [`docs/手册/03-器件任务书.md`](docs/手册/03-器件任务书.md) §7。
