#!/usr/bin/env python3
"""基础版**接线文档生成器 / 校验器**（`basic/hardware/` 里的文档由它产出）。
...
basic/hardware/*.md    （全部文档；份数由 DOCUMENTS 决定，别在文案里写死数字）
```

用法::

    python3 basic/tools/wire_docs.py --generate   # 重新生成全部文档
...
# ==========================================================================
# 接口接线表（只要"树莓派接线 + 元件接线"两张表）
# ==========================================================================


def build_interface_tables() -> str:
    """**两张表就说完接线**：① 树莓派这头每个脚接什么 ② 元件那头每个针脚接到哪。

    为什么要单独再来一份（用户 2026-09-25 要求"只要树莓派接线与元件接线两个接口"）：
    01/02/03/04 那几份为了"讲清楚"带了很多旁支（40-pin 全表、供电安全、线色自查、现象对照），
    上机时**真正要照着插的只有两张表**。这一份把它们单独抽出来，一页看完。

    事实仍然**全部来自 `wire_spec`**（引脚号、BCM、线色、上拉文字、供电方式），
    所以它与其它文档**不可能对不上** —— 改了 `wire_spec` 忘了改这里，`--check` 会当场报错。
    """
    ...
    module_rows = [
        ["`VCC` / `+` / `VDD`", "**3.3V**", wire_spec.V33_PHYSICAL, "—",
         "**绝不要接 5V**（脚 2/4）：5V 会把数据电平一起拉高，伤 GPIO"],
        ["`DATA` / `OUT` / `S`", "**DATA**", [int(wire_spec.DATA_PHYSICAL or 0)], f"GPIO{wire_spec.DATA_BCM}",
         f"单总线（双向）；裸四针必须外接 {wire_spec.pullup_text()} 上拉到 3.3V"],
        ["`GND` / `-`", "**GND**", wire_spec.GND_PHYSICAL, "—",
         f"任意一个地脚都行（推荐脚 {wire_spec.GND_RECOMMENDED}）；**必须共地**"],
    ]
    module_rows = [
        [pin, signal, "/".join(str(p) for p in physicals), bcm, note]
        for pin, signal, physicals, bcm, note in module_rows
    ]
    return f"""{_header("06 · 接口接线表（树莓派接线 + 元件接线）")}
...
本基础版只动 **3 个脚**（其余 37 个全空闲）：**脚 1 = 3.3V、脚 6 = GND、脚 {wire_spec.DATA_PHYSICAL} = DATA**。
...
> （GPIO{wire_spec.DATA_BCM} = 物理脚 {wire_spec.DATA_PHYSICAL}，但 GPIO27 = 物理脚 13）。
...
1. **断电**，按第 1 节挑好三个脚（脚 1 / 脚 6 / 脚 {wire_spec.DATA_PHYSICAL}）；
...
# ==========================================================================
# 打印合集（把全部文档合成一份可打印的 Markdown → PDF）
# ==========================================================================
...
def print_sources() -> Dict[str, Tuple[str, ...]]:
    ...
    两类：
    1. 单份文档的 PDF（`04-线色与自查卡.md` 一页纸、`06-接口接线表.md` 两张表）；
    2. **合集 PDF**（由全部文档合成，源就是合集自身的 md）。
    """
    return {
        "04-线色与自查卡.md": ("04-线色与自查卡.pdf", "04-线色与自查卡.html"),
        "06-接口接线表.md": ("06-接口接线表.pdf", "06-接口接线表.html"),
...
def build_collection() -> str:
    """把全部文档合成一份适合打印的 Markdown（含封面 + 用法 + 各章）。
...
DOCUMENTS: Dict[str, Callable[[], str]] = {
    ...
    "06-接口接线表.md": build_interface_tables,
}

#: 生成/校验时必须存在的其它文件（打印产物与其中间 HTML，由 `scripts/md2pdf.cjs` 产出）
REQUIRED_FILES: Tuple[str, ...] = (
    "04-线色与自查卡.pdf", "04-线色与自查卡.html",
    "06-接口接线表.pdf", "06-接口接线表.html",
