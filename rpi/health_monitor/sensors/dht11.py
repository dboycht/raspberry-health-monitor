...
class Dht11(Device):
    ...
    def _read_lgpio_raw(self) -> Tuple[float, float]:
        ...
        #    ⚠️ 关键坑一（2026-09-24 用合成时序做单测时抓到，真机上表现为"校验和不符"）：
        ...
        #
        #    ⚠️⚠️ 关键坑二（2026-09-25 真机抓到，稳定表现为"只解出 39/40 个 bit"）：
        #    一帧的边沿序列**可能是 83 个而不是 82 个** —— 多出来的那一个是
        #    **帧尾"上拉把总线拉回空闲"的上升沿**（DHT11 最后一位结束后，从机上拉
        #    先把线拉高；实测时间戳：最后 6 个边沿是 …低 3749.1 → 高 3802.9 µs）。
        #    它**不是数据位**，但会让"从下标 1 开始逐对扫"的写法整体错位一格：
        #      实测数据：上升沿 42 个、下降沿 41 个 ⇒ 只能配出 40 对（应答 + 39 位）
        #      ⇒ 丢掉"应答"后只剩 **39** 位 ⇒ 帧解析必然失败。
        #    修法：**不猜下标，直接取"最后 41 对"**（应答 1 对 + 数据 40 对），
        #    并把尾部那个落单的上升沿丢掉。判据可执行：一帧的 (高,低) 对恒为 41 对。
        ...
        #: 一帧应有的高电平段数：应答 1 对 + 40 个数据位 = 41 对
        PAIRS_PER_FRAME = 41
        ...
        if len(widths_us) < PAIRS_PER_FRAME:
            # 对不上就报出"实际配出几对"，这比"只解出 N 位"更接近根因，便于下次排查
            raise DeviceIOError(
                f"DHT11（GPIO{self.pin}）帧结构不完整：只配出 {len(widths_us)} 个高电平段"
                f"（一帧应为 {PAIRS_PER_FRAME} 个 = 应答 1 + 数据 40）⇒ 数据位不足。"
                "常见原因：上拉偏弱/线太长导致漏采边沿；或两次读取间隔不足 2 秒"
            )
        # 取**最后 41 对**：尾部那个"上拉回空闲"的上升沿会被自然丢掉
        widths_us = widths_us[-PAIRS_PER_FRAME:]
