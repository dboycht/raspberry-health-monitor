...
def diag_adc() -> None:
    ...
    try:
        ...
        if statistics.mean(ch1_high) > 1000:
            print("    ✅ CH1 接 3.3V 后接近满量程 → **通道选择正常、CH1 也能读**")
        else:
            print("    ⚠️ CH1 接 3.3V 后没变化 → MCP3002 的通道选择没生效（或 CH1 没接出来）")
        ...
        if statistics.mean(ch1_low) < 50:
            print("    ✅ CH1 接 GND 后接近 0 → 通道工作正常，接回悬空即可")
        else:
            print("    ⚠️ CH1 接 GND 后仍不为 0 → 接线或芯片有问题")

        print("\n  【第 4 步】诊断 CH0 上的 TMP36：")
        avg0 = statistics.mean(read_adc(spi, 0, 10))
        volts0 = avg0 / 1023 * 3.3
        print(f"    CH0 平均 raw={avg0:.1f} → {volts0:.3f} V")
        print(f"    按 TMP36 标准公式：(V − 0.5) / 0.01 = {(volts0 - 0.5) / 0.01:.1f} ℃")
        print("    正常室温应 20~30 ℃。对不上时按顺序查：")
