...
def main() -> int:
    ...
    if cfg.resolved_key():
        safe_print("  ✅ 密钥已解析到（来自配置文件或环境变量）")
    else:
        safe_print(f"  ❌ 没有可用密钥：设 {env_name} 或在 devices.local.json 里写 access_key")
        return 2
    ...
    if accepted:
        ...
        safe_print("  ❌ 平台拒绝（rejected）：常见原因是数据流未创建（旧版无需创建）、"
              "token 过期、或 payload 里有非法值")
        code = 1
    else:
        safe_print("  ⚠️ 已发布但没等到回执；请到控制台确认数据流是否更新")
        code = 0
    pub.stop()
    return code
